...
class Tool:
    def __init__(self, dir=os.path.dirname(__file__), model='claude-3-5-haiku-latest', tool_tag = tool_tag):
        ...
        self.commands = {
        'write': lambda text, filename: self.write(text, filename, False),
        'overwrite': lambda text, filename: self.write(text, filename, True),
        'read': lambda filename: self.read(filename),
